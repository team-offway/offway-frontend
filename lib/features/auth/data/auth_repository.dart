...
class AuthRepository {
  ...
  /// 그 계정의 토큰을 전부 끊는다. 다만 회전 뒤 **10초 안**의 재사용은 응답
  /// 유실로 보고 새 쌍을 다시 준다(core #411) — 아래에서 끊김·5xx에 토큰을
  /// 남겨 두는 것이 그 창 안의 재시도로 이어진다.
  ///
  ...
  Future<void> logout() async {
    // 이 기기의 refresh를 함께 보낸다(core #411) — 서버가 **그 세션만** 끊는다.
    // 안 실으면 그 사용자의 모든 기기가 끊긴다(옛 앱 호환 경로) — 예전엔 그게
    // 기본이라 시뮬레이터나 백오피스에서 로그아웃하면 최대 1시간 뒤 폰까지 풀렸다
    final refresh = await _storage.refreshToken;
    try {
