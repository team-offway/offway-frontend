...
class _CourseScreenState extends ConsumerState<CourseScreen> {
  ...
  Future<void> _saveToMyCourses(Map<String, dynamic> course) async {
    ...
    try {
      // 출발지 이름을 앱이 만들지 않는다 — 좌표를 더 이상 다루지 않으므로
      // 변환할 것이 없다. 서버가 `originCode` 로 이름을 알아 자차 도착
      // 안내의 `fromPlace` 를 채운다(core #591)
  ...
  /// (날짜·연차 뱃지 없이 안내 문구).
  Future<void> _saveImage(Map<String, dynamic> course, int? day) =>
      saveCourseImage(
        context,
