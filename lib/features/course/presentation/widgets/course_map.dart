...
/// DS 핀 아이콘(단색). 코스확정 화면의 ic_pin_location(반투명 테두리)과는
/// 다른 파일이다 — 지도 위에서는 반투명이 배경에 묻힌다
const _singlePinAsset = 'assets/icons/ic_map_pin.svg';
  ...
  Widget build(BuildContext context) {
    ...
    return NaverMap(
      ...
      onMapReady: (controller) async {
        ...
        if (single) {
          // SVG는 첫 그리기에서 비동기로 로드된다 — fromWidget이 그 전에
          // 캡처하면 핀이 빈 이미지가 되므로, 미리 캐시에 넣어 첫 프레임부터
          // 동기로 그려지게 한다
          const loader = SvgAssetLoader(_singlePinAsset);
        ...
        for (var i = 0; i < places.length; i++) {
          ...
          // 장소가 하나면 DS 핀 에셋을 쓴다 — 네이버 기본 핀의 iconTintColor는
          // **가산 혼합**이라 초록 원본 위에 어떤 파랑을 얹어도 밝은 청록이
          // 되어 우리 색이 나오지 않는다
